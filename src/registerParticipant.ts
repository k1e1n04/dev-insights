...
export function registerParticipant(context: vscode.ExtensionContext) {
	...
	const handler: vscode.ChatRequestHandler = async (
		...
		try {
			...
			// エラーハンドリング
			vscode.window.showErrorMessage(`チャットリクエストが失敗しました: ${err}`);
			stream.markdown(
	...
	const catParticipant = vscode.chat.createChatParticipant(CAT_PARTICIPANT_ID, handler);
	catParticipant.iconPath = vscode.Uri.joinPath(context.extensionUri, 'icon.png');
